    ...
    <AdminLayout>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-3">
        ...
        <aside className="space-y-6">
          ...
          <section className="rounded-2xl border border-burgundy-100 bg-white p-6"><h2 className="font-serif text-2xl font-semibold text-ink">Customer & delivery</h2><ul className="mt-4 space-y-3 text-sm"><li className="flex items-start gap-2.5"><BuildingIcon className="mt-0.5 h-4 w-4 text-burgundy-700" /><div><p className="font-medium text-ink">{order.business}</p><p className="text-ink/60">{order.contactName}</p></div></li><li className="flex items-center gap-2.5"><MailIcon className="h-4 w-4 text-burgundy-700" />{order.email}</li><li className="flex items-center gap-2.5"><PhoneIcon className="h-4 w-4 text-burgundy-700" />{order.phone}</li><li className="flex items-start gap-2.5"><FileTextIcon className="mt-0.5 h-4 w-4 text-burgundy-700" />Trading licence: {order.licenseNo}</li><li className="flex items-start gap-2.5"><MapPinIcon className="mt-0.5 h-4 w-4 text-burgundy-700" /><span className="whitespace-pre-line">{order.deliveryAddress}</span></li>{order.deliveryDate && <li className="flex items-center gap-2.5"><CalendarDaysIcon className="h-4 w-4 text-burgundy-700" />Requested {formatDate(order.deliveryDate)}</li>}</ul></section>
