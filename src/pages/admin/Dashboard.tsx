  ...
  return (
    <AdminLayout>
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-3">
        <section className="rounded-2xl border border-burgundy-100 bg-white p-6 lg:col-span-2"><div className="flex items-center justify-between"><h2 className="font-serif text-2xl font-semibold text-ink">Fulfilment queue</h2><Link to="/portal/orders" className="inline-flex items-center gap-1 text-sm font-semibold text-burgundy-800 hover:text-burgundy-900">View all <ArrowRightIcon className="h-4 w-4" /></Link></div>{orders.length === 0 ? <p className="mt-6 text-sm text-ink/50">No orders yet. Submitted orders will land here.</p> : <div className="mt-4 divide-y divide-burgundy-50">{orders.slice(0, 6).map((order) => <Link key={order.id} to={`/portal/orders/${order.id}`} className="flex items-center justify-between py-3 hover:bg-cream/60"><div><p className="font-medium text-ink">{order.reference}</p><p className="text-xs text-ink/50">{order.contactName} · {formatDate(order.createdAt)}</p></div><div className="flex items-center gap-4"><span className="hidden text-sm font-semibold sm:inline">{formatCurrency(order.total)}</span><StatusBadge status={order.status} /></div></Link>)}</div>}</section>
        ...
        <section className="rounded-2xl border border-burgundy-100 bg-white p-6 lg:col-span-3"><div className="flex items-center justify-between"><h2 className="font-serif text-2xl font-semibold text-ink">Low stock watchlist</h2><Link to="/portal/inventory" className="text-sm font-semibold text-burgundy-800 hover:text-burgundy-900">Manage inventory</Link></div>{lowStock.length === 0 ? <p className="mt-5 text-sm text-ink/50">All items are above their stock threshold.</p> : <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{lowStock.map((product) => <div key={product.id} className="rounded-xl bg-amber2-50 p-4"><p className="font-medium text-ink">{product.name}</p><p className="mt-1 text-xs text-ink/55">{product.brand}</p><p className="mt-3 text-sm font-bold text-amber2-800">{product.stockUnits} pieces left</p></div>)}</div>}</section>
      </div>
    </AdminLayout>);
