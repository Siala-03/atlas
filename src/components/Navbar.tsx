...
import { SPIRITS_DROPDOWN, WINE_DROPDOWN, BEER_DROPDOWN } from "../lib/categoryTaxonomy";

const MOBILE_LINKS = [
{ to: "/shop", label: "Shop All" },
{ to: "/shop?family=Spirits", label: "Spirits" },
      ...
      <header className="w-full border-b border-burgundy-100 bg-cream/95 backdrop-blur">
        <div className="mx-auto flex h-32 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          <Link to="/" className="flex items-center gap-3">
            <Logo className="h-28" />
            <span className="hidden font-serif text-sm italic text-burgundy-700/80 sm:block">
          ...
          <nav className="hidden items-center gap-8 lg:flex">
            <NavLink
              ...
              isActive ? "text-burgundy-800" : "text-ink/70"}`
              }>

              Shop All
            </NavLink>
            <NavDropdown label="Spirits" to="/shop?family=Spirits" items={SPIRITS_DROPDOWN} />
            <NavDropdown label="Wine" to="/shop?category=Wine" items={WINE_DROPDOWN} />
            <NavDropdown label="Beer" to="/shop?category=Beer" items={BEER_DROPDOWN} />
          </nav>
          ...
          <div className="flex items-center gap-3">
            ...
            <Link
              to="/account"
              className="hidden h-11 w-11 items-center justify-center rounded-full text-burgundy-800 hover:bg-burgundy-50 sm:inline-flex"
              aria-label="Account">
            ...
            className="overflow-hidden border-t border-burgundy-100 bg-cream lg:hidden">
              ...
              <nav className="mx-auto flex max-w-7xl flex-col px-4 py-3">
                <div className="flex items-center justify-between px-3 py-2">
                  ...
                  className="inline-flex items-center gap-1.5 text-sm font-medium text-ink/70">
